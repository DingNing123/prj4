...
class MIB(BertPreTrainedModel):
    ...
    def forward(
        ...
        # 我想这里是因为设计了多个损失函数，有多个计算图分支，因此需要
        # retain_graph = True 保留计算图，以便更新每个分支的梯度 。
...
class fusion(nn.Module):
    ...
    def encode_l(self, x):
        """
        x : [batch_size,784]  公式9 用各自的神经网络拟合均值和方差
        """
        x = self.encoder_l(x)
    ...
    def reparameterise(self, mu, std):
        ...
        """        
        # get epsilon from standard normal 公式10 
    ...
    def loss_function(self, y_pred, y, mu, std):
        ...
        loss_fct = L1Loss()
        # 公式11 ，15 ; -｜y-^y| - beta*KL() make it max i.e. make  CE + beta*KL min  ;  mu^2+std^2-2log(std)-1 ???
        # https://www.cnblogs.com/huangshiyu13/p/6898212.html 解释了当N2 服从正态分布时，KL散度的计算是
        # 1/2（mean^2 + std^2 -2log(mean) -1)
        # 论文中的公式是求最大值，神经网络的代码中使用最小值优化。因此公式应该为
        # beta*KL-logq(y|z)   1/n 被我省略了，因为逻辑嵌套在了神经网络的代码中
        # 而logq(y|z) 是回归任务时 为 -|y-^y|, 因此代码中CE符号转化为正 
